...
Демо-сцена `/world`: 1000 движущихся агентов. Симуляция живёт в ECS (koota), картинка — в R3F, кнопки и пауза — в React + Zustand.
...
Открой `/world`. Three.js грузится только на клиенте (`ssr: false` в `WorldAppLoader`).
...
Текущие traits: `Agent`, `Position`, `Velocity`, `Color`, `Instance`, `Wander`, `Time` (singleton мира), `SwarmMesh` (ссылка на Three.js mesh).
...
Для одного героя или маркера можно отдельный `<mesh>` и `useTrait(entity, Position)`. Для сотен одинаковых мешей — только instancing, как `swarm-instances.tsx`.
