...
export function PlayHud() {
  ...
  const { spawnSwarm, resetPlay } = useActions(playActions);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.code === "KeyR") {
        event.preventDefault();
        resetPlay(SWARM_COUNT);
      }
    };

    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [resetPlay, togglePaused]);
    ...
    <div className="pointer-events-none absolute inset-0 z-10 p-4">
      ...
      >
        <CardHeader>
          ...
          <CardDescription>
            WASD or click the ground. NPCs swarm.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="secondary">{agents.length} npc</Badge>
          ...
          <div className="flex items-center justify-between gap-3">
            <Label htmlFor="play-paused">Pause</Label>
            <Switch
              id="play-paused"
              checked={paused}
          ...
          <div className="flex items-center justify-between gap-3">
            <Label htmlFor="play-grid">Grid</Label>
            <Switch
              id="play-grid"
              checked={showGrid}
          ...
          <div className="flex gap-2">
            ...
            <Button className="flex-1" onClick={() => resetPlay(SWARM_COUNT)}>
              Reset
            </Button>
          </div>

          <p className="text-muted-foreground text-xs">
            <Kbd>WASD</Kbd> move · click ground · <Kbd>Space</Kbd> pause ·{" "}
            <Kbd>R</Kbd> reset
          </p>
        </CardContent>
      </Card>
